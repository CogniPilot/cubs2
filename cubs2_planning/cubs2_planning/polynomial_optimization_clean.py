...
def plot_piecewise_polynomials(
    polynomials, segment_times, t_eval=None, title="Piecewise Polynomial"
):
    """
    ...
    for i, t in enumerate(t_eval):
        for k in range(len(polynomials)):
            if gamma[k] <= t <= gamma[k + 1]:
                ...
                velocity[i] = (
                    np.polyval(derivatives[k][1][::-1], t_local)
                    if len(derivatives[k][1]) > 0
                    else 0
                )
                acceleration[i] = (
                    np.polyval(derivatives[k][2][::-1], t_local)
                    if len(derivatives[k][2]) > 0
                    else 0
                )
                jerk[i] = (
                    np.polyval(derivatives[k][3][::-1], t_local)
                    if len(derivatives[k][3]) > 0
                    else 0
                )
                break
...
def run_poly_optimization(
    order, tau, segments, weights, boundary_conditions, continuity
):
    """
    ...
    print(f"Constraint vector shape: {b.shape}")
    ...
    print(f"  Cost matrix shape: {Q.shape}")
    ...
    return {"polys": polys, "order": order, "segments": segments, "tau": tau}
...
if __name__ == "__main__":
    ...
    boundary_positions = [[0.0, None, None, 0.0]]  # Position: start=0, end=0
    boundary_velocities = [[0, None, None, None]]  # Velocity: start=0, others free
    boundary_accelerations = [[0, None, None, None]]  # Acceleration: start=0
    boundary_jerk = [[0, None, None, None]]  # Jerk: start=0
    boundary_snap = [[0, None, None, None]]  # Snap: start=0
