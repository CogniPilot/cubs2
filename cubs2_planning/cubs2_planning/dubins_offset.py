...
from polynomial_optimization import (
    run_poly_optimization,
    plot_piecewise_polynomials,
    plot_derivatives,
)
...
def evaluate_dubins_path(
    plan_info, start_pos, start_yaw, end_pos, end_yaw, turn_radius, num_points=1000
):
    """
...
def plot_curvature(path_points, curvature, title=""):
    ...
    for i in range(1, len(curvature)):
        arc_length[i] = arc_length[i - 1] + np.linalg.norm(
...
def plot_dubins_path(
    ax, path_points, plan_info, start_pos, end_pos, turn_radius, label=""
):
    """
...
def rolling_median_replace(x, window=7, thresh=3.0, replace_with="median"):
    ...
    if replace_with == "median":
        ...
        good = ~outliers
        x_clean[outliers] = np.interp(
            np.flatnonzero(outliers), np.flatnonzero(good), x[good]
        )
    return x_clean
...
if __name__ == "__main__":
    ...
    for i in range(len(waypoints) - 1):
        ...
        plan = plan_dubins_path(
            wp_start["pos"], wp_start["yaw"], wp_end["pos"], wp_end["yaw"], turn_radius
        )
    ...
    ax1.set_title(
        "Multi-Waypoint Dubins Path (6 Waypoints)", fontsize=13, fontweight="bold"
    )
    ax1.grid(True, alpha=0.3)
    ax1.axis("equal")
    ...
    boundary_accelerations = [
        [start_condition, *([None] * (segments - 1)), end_condition]
    ...
    fig1, ax1 = plot_piecewise_polynomials(
        outputs["polys"], tau, title="Optimized Piecewise Polynomial Trajectory"
    )
    plt.tight_layout()
    plt.show()
