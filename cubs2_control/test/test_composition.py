...
def test_closed_loop_trim_5ms():
    ...
    ail_manual, elev_manual, rud_manual, thr_manual, mode = (
        u_trim[0],
        u_trim[1],
        u_trim[2],
        u_trim[3],
    ...
    print(
        f"Manual inputs: ail={float(ail_manual):.4f}, elev={float(elev_manual):.4f}, rud={float(rud_manual):.4f}, thr={float(thr_manual):.4f}"
    )
...
def test_closed_loop_matches_open_loop():
    ...
    print(
        f"   Attitude: phi={np.degrees(phi_trim):.2f}°, theta={np.degrees(theta_trim):.2f}°, psi={np.degrees(psi_trim):.2f}°"
    )
    print(
        f"   Controls: ail={float(u_plant_trim.ail):.4f}, elev={float(u_plant_trim.elev):.4f}, rud={float(u_plant_trim.rud):.4f}, thr={float(u_plant_trim.thr):.4f}"
    )
    ...
    print(
        f"   Controller outputs: ail={ail_out:.4f}, elev={elev_out:.4f}, rud={rud_out:.4f}, thr={thr_out:.4f}"
    )
    ...
    print(
        f"  Aileron: {u_error_ail:.4f} (open-loop: {float(u_plant_trim.ail):.4f}, closed-loop: {ail_out:.4f})"
    )
    print(
        f"  Elevator: {u_error_elev:.4f} (open-loop: {float(u_plant_trim.elev):.4f}, closed-loop: {elev_out:.4f})"
    )
    print(
        f"  Rudder: {u_error_rud:.4f} (open-loop: {float(u_plant_trim.rud):.4f}, closed-loop: {rud_out:.4f})"
    )
    print(
        f"  Throttle: {u_error_thr:.4f} (open-loop: {float(u_plant_trim.thr):.4f}, closed-loop: {thr_out:.4f})"
    )
    ...
    controls_match = (
        u_error_ail < control_tol
        and u_error_elev < control_tol
        and u_error_rud < control_tol
        and u_error_thr < control_tol
    )
