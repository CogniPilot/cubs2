...
class AutolevelController:
    ...
    stick_rate_scale_theta: float = param(
        2.0 * np.pi, desc="stick to pitch rate (rad/s)"
    )
...
def autolevel_controller() -> Model:
    ...
    p_cmd_level = (
        -m.Kp_phi.sym * phi
    )  # Roll error: always drives to phi=0 (wings-level)
    q_cmd_level = (
        -m.Kp_theta.sym * theta
    )  # Pitch error: always drives to theta=0 (level)
    ...
    angle_margin = 0.9
    phi_saturation_factor = cy.fmin(
        1.0, (m.phi_max.sym - cy.fabs(phi)) / (m.phi_max.sym * (1.0 - angle_margin))
    )
    theta_saturation_factor = cy.fmin(
        1.0,
        (m.theta_max.sym - cy.fabs(theta)) / (m.theta_max.sym * (1.0 - angle_margin)),
    )
    ...
    p_cmd = (
        p_cmd_stick_limited * (1.0 - m.mode.sym)
        + (p_cmd_stick_limited + p_cmd_level) * m.mode.sym
    )
    q_cmd = (
        q_cmd_stick_limited * (1.0 - m.mode.sym)
        + (q_cmd_stick_limited + q_cmd_level) * m.mode.sym
    )
    ...
    elev_stabilized = _saturate(
        m.Kp_q.sym * e_q + m.Ki_q.sym * m.i_q.sym, m.elev_min.sym, m.elev_max.sym
    )
