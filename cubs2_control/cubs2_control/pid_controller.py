...
@symbolic
class PIDInputs:
    roll_ref: ca.SX = input_var(0.0, "roll rate ref (rad/s)")
    pitch_ref: ca.SX = input_var(0.0, "pitch rate ref (rad/s)")
    yaw_ref: ca.SX = input_var(0.0, "yaw rate ref (rad/s)")
    speed_ref: ca.SX = input_var(0.0, "speed ref (m/s)")
    roll_meas: ca.SX = input_var(0.0, "roll rate meas (rad/s)")
    pitch_meas: ca.SX = input_var(0.0, "pitch rate meas (rad/s)")
    yaw_meas: ca.SX = input_var(0.0, "yaw rate meas (rad/s)")
    speed_meas: ca.SX = input_var(0.0, "speed meas (m/s)")
    d_roll_meas: ca.SX = input_var(0.0, "d/dt roll rate (rad/s^2)")
    d_pitch_meas: ca.SX = input_var(0.0, "d/dt pitch rate (rad/s^2)")
    d_yaw_meas: ca.SX = input_var(0.0, "d/dt yaw rate (rad/s^2)")
    d_speed_meas: ca.SX = input_var(0.0, "d/dt speed (m/s^2)")
...
def pid_controller() -> ModelSX:
    ...
    y.ail = model.saturate(
        p.Kp_roll * e_roll + p.Ki_roll * x.i_roll - p.Kd_roll * u.d_roll_meas, p.ail_min, p.ail_max
    )
    ...
    y.rud = model.saturate(
        p.Kp_yaw * e_yaw + p.Ki_yaw * x.i_yaw - p.Kd_yaw * u.d_yaw_meas, p.rud_min, p.rud_max
    )
