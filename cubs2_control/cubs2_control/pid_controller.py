...
class PIDController:
    ...
    # === Parameters ===
    Kp_roll: float = param(2.5, 'P gain roll')
    Ki_roll: float = param(0.8, 'I gain roll')
    Kd_roll: float = param(0.2, 'D gain roll')
    Kp_pitch: float = param(3.0, 'P gain pitch')
    Ki_pitch: float = param(0.9, 'I gain pitch')
    Kd_pitch: float = param(0.25, 'D gain pitch')
    Kp_yaw: float = param(1.8, 'P gain yaw')
    Ki_yaw: float = param(0.5, 'I gain yaw')
    Kd_yaw: float = param(0.15, 'D gain yaw')
    Kp_speed: float = param(0.7, 'P gain speed')
    Ki_speed: float = param(0.3, 'I gain speed')
    Kd_speed: float = param(0.05, 'D gain speed')
...
def pid_controller() -> Model:
    ...
    Returns:
        Model: PID controller model with integral states and control outputs
    """
    ...
    # PID outputs
    ail_out = _saturate(
        m.Kp_roll.sym * e_roll + m.Ki_roll.sym * m.i_roll.sym - m.Kd_roll.sym * m.d_roll_meas.sym,
        m.ail_min.sym,
        m.ail_max.sym,
    )
    elev_out = _saturate(
        m.Kp_pitch.sym * e_pitch + m.Ki_pitch.sym * m.i_pitch.sym - m.Kd_pitch.sym * m.d_pitch_meas.sym,
        m.elev_min.sym,
        m.elev_max.sym,
    )
    rud_out = _saturate(
        m.Kp_yaw.sym * e_yaw + m.Ki_yaw.sym * m.i_yaw.sym - m.Kd_yaw.sym * m.d_yaw_meas.sym,
        m.rud_min.sym,
        m.rud_max.sym,
    )
    thr_out = _saturate(
        m.Kp_speed.sym * e_speed + m.Ki_speed.sym * m.i_speed.sym - m.Kd_speed.sym * m.d_speed_meas.sym,
        m.thr_min.sym,
        m.thr_max.sym,
    )
