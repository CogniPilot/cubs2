...
class TestSportCubNode:
    ...
    def test_initial_conditions_near_origin(self):
        ...
        np.testing.assert_allclose(
            x.r, [1.0, 0.0, 0.0, 0.0], atol=1e-6)  # quaternion in x.r
    ...
    def test_ground_reaction_settles_at_origin(self):
        ...
        settled_mask = (v_mags[half_idx:] < 0.05) & (
        ...
        assert (xy_distance < 0.05), f'Aircraft drifted horizontally {
            xy_distance:.3f} m (max drift {max_xy_drift:.3f} m).'
    ...
    def test_ground_bounce_characterization(self):
            ...
            for step in range(1, len(z_series)):
                ...
                if first_contact and not rebound_found:
                    ...
                    if (
                        prev_vz > 0.001 and
                        vz <= 0 and
                        (z - min_z_after_contact) >= bounce_threshold
                    ):
                        rebound_found = True
                        rebound_height = z
                prev_vz = vz
        ...
        print(
            f"{'h0':>6} {'rebound':>10} {'e':>8} {
                'penetration':>12} {'settle_s':>10} {'settled':>8}"
        )
        for h0, rebound_height, e, max_penetration, settling_time, settled in results:
            settling_str = f'{
                settling_time:.2f}' if settling_time is not None else 'None'
            print(
                f'{
                    h0:6.2f} {
                    rebound_height:10.4f} {
                    e:8.4f} {
                    max_penetration:12.4f} {
                        settling_str:>10} {
                            str(settled):>8}')

        for h0, _, e, max_penetration, settling_time, settled in results:
            assert e <= 1.0 + 1e-6
            # Coefficient of restitution bounds (softer for low drops which may
            # not bounce)
            if h0 >= 1.0:
                assert e <= 0.55, f'Bounce too energetic for h0={
                    h0}m: e={e:.3f}'
            ...
            # Tighter penetration limits (wheel_offset accounts for wheel
            # position)
            if h0 <= 1.0:
                ...
                max_allowed = 0.25
            assert (wheel_penetration < max_allowed), f'Excessive penetration for h0={
                h0}m: {wheel_penetration:.3f}m > {max_allowed}m'
    ...
    def test_glide_slope(self):
        ...
        print(f'  CL = {CL_trim:.3f}, CD = {
              CD_trim:.3f}, L/D = {CL_trim / CD_trim:.2f}')
        ...
        # Assert trim quality
        assert v_dot_mag < 0.01, f'Velocity acceleration not zero: |v_dot|={
            v_dot_mag:.6f}'
        assert w_dot_mag < 0.01, f'Angular acceleration not zero: |w_dot|={
            w_dot_mag:.6f}'
        ...
        horizontal_distances = np.array(
            [np.linalg.norm(p[:2] - x0_horizontal) for p in positions])
        ...
        max_pitch = float(np.max(np.abs(pitch_angles))
        ...
        # The trim should be stable enough that velocity and pitch don't
        # diverge significantly
        assert max_roll < 10.0, f'Roll control failed: max_roll={
            max_roll:.1f}°'
        assert airspeed_std < 1.0, f'Airspeed not stable: std={
            airspeed_std:.2f} m/s'
        assert pitch_std < 5.0, f'Pitch not stable: std={pitch_std:.2f}°'

        # Tighter performance bounds - should achieve good glide ratio
        assert glide_ratio > 5.5, f'Glide ratio too low: {
            glide_ratio:.2f}:1 (expect 6-11:1)'
        assert glide_ratio < 12.0, f'Glide ratio unrealistically high: {
            glide_ratio:.2f}:1'
        ...
        assert 4.5 < avg_airspeed < 8.0, f'Average airspeed out of band: {
            avg_airspeed:.2f} m/s'
        assert min_airspeed > 4.0, f'Minimum airspeed too low: {
            min_airspeed:.2f} m/s'
        ...
        assert max_alpha_deg < 12.0, f'Angle of attack too high: {
            max_alpha_deg:.1f}°'

        # Descent rate should be reasonable
        assert 0.5 < descent_rate < 2.0, f'Descent rate out of range: {
            descent_rate:.2f} m/s'
    ...
    def test_trim_linearize_5ms(self):
        ...
        # Always force verbose printing for this test so optimizer & mode
        # details are visible
        x_trim, u_trim, _ = find_trim(
            model,
            V_target=5.0,
            ...
            # final solver summary only (no iteration table)
            ipopt_print_level=1,
        )
        A, B = linearize_dynamics(model, x_trim, u_trim)
        assert A.shape == (len(x_trim), len(x_trim))
        assert B.shape[0] == len(x_trim)
        # Build state names dynamically if index map available (fallback to
        # legacy constant)
        ...
        outputs = self._extract_outputs(
            model, x_trim, u_trim, model.p0.as_vec())
        F_b = outputs.F_b
        M_b = outputs.M_b
        print(f'  F_b = {F_b}')
        print(f'  M_b = {M_b}')
        ...
        x_dot = model.f_x(x_trim, u_trim, model.p0.as_vec())
        # index 5 in concatenated (p_dot[0:3], v_dot[3:6], ...)
        v_dot_z = float(x_dot[5])
        v_z = float(x_trim[5])
        assert abs(v_z) < 0.02, f'Vertical velocity not trimmed: {v_z:.4f} m/s'
        assert abs(v_dot_z) < 0.02, f'Vertical acceleration high: {
            v_dot_z:.4f} m/s^2'
        ...
        assert abs(outputs.Vt -
                   5.0) < 0.2, f'Trim velocity error: {outputs.Vt:.2f} m/s'
    ...
    def test_euler_linearization_5ms(self):
        ...
        # Verify state structure - should have Euler angles (3D) instead of
        # quaternion (4D)
        x_state = model_euler.state_type.from_vec(x_trim)
        assert hasattr(
            x_state, 'r'), 'State should have 'r' field for rotation'
        assert len(x_state.r) == 3, f'Euler angles should be 3D, got {
            len(x_state.r)}'

        # Check that Euler angles are reasonable for level flight
        psi, theta, phi = x_state.r[0], x_state.r[1], x_state.r[2]
        print(
            f'  Trim Euler angles: psi={np.degrees(psi):.2f}°, theta={
                np.degrees(theta):.2f}°, phi={np.degrees(phi):.2f}°'
        )

        # For level flight, pitch should be small positive, roll near zero
        assert abs(phi) < np.deg2rad(5.0), f'Roll angle too large: {
            np.degrees(phi):.2f}°'
        assert abs(theta) < np.deg2rad(15.0), f'Pitch angle too large: {
            np.degrees(theta):.2f}°'
        ...
        # position + velocity + euler + angular velocity
        expected_state_size = 3 + 3 + 3 + 3
        assert A.shape == (
            expected_state_size,
            expected_state_size,
        ), f'A matrix size mismatch: {A.shape}'
        assert B.shape[0] == expected_state_size, f'B matrix rows mismatch: {
            B.shape}'
        ...
        # Verify forces and moments at trim
        outputs = self._extract_outputs(
            model_euler, x_trim, u_trim, model_euler.p0.as_vec())
        F_b = outputs.F_b
        M_b = outputs.M_b
        ...
        LD = (
            outputs.CL / outputs.CD if outputs.CD > 0 else 0)
        assert 5.0 < LD < 10.0, f'L/D out of range: {LD:.2f}'
    ...
    def test_trim_quaternion_vs_euler_5ms(self):
        ...
        np.testing.assert_allclose(
            state_quat.p, state_euler.p, rtol=1e-3, atol=1e-3)
        ...
        print(
            f'  From Quat:  phi={
                np.degrees(phi_from_quat):.3f}°, theta={
                np.degrees(theta_from_quat):.3f}°, psi={
                np.degrees(psi_from_quat):.3f}°')
        print(
            f'  Euler:      phi={
                np.degrees(
                    state_euler.r[2]):.3f}°, theta={
                np.degrees(
                    state_euler.r[1]):.3f}°, psi={
                        np.degrees(
                            state_euler.r[0]):.3f}°')
        ...
        np.testing.assert_allclose(
            state_quat.w, state_euler.w, rtol=2e-3, atol=7e-3)
        ...
        print(
            f'  Quaternion: ail={
                input_quat.ail:.4f}, elev={
                input_quat.elev:.4f}, rud={
                input_quat.rud:.4f}, thr={
                    input_quat.thr:.4f}')
        print(
            f'  Euler:      ail={
                input_euler.ail:.4f}, elev={
                input_euler.elev:.4f}, rud={
                input_euler.rud:.4f}, thr={
                    input_euler.thr:.4f}')
        np.testing.assert_allclose(
        ...
        print(
            f'  Quaternion: Vt={
                outputs_quat.Vt:.3f} m/s, alpha={
                np.degrees(
                    outputs_quat.alpha):.3f}°, CL={
                    outputs_quat.CL:.4f}, CD={
                        outputs_quat.CD:.4f}')
        print(
            f'  Euler:      Vt={
                outputs_euler.Vt:.3f} m/s, alpha={
                np.degrees(
                    outputs_euler.alpha):.3f}°, CL={
                    outputs_euler.CL:.4f}, CD={
                        outputs_euler.CD:.4f}')
        np.testing.assert_allclose(
            outputs_quat.Vt, outputs_euler.Vt, rtol=1e-3)
        np.testing.assert_allclose(
            outputs_quat.alpha,
            outputs_euler.alpha,
            rtol=1e-2,
            atol=np.deg2rad(0.5))
        np.testing.assert_allclose(
            outputs_quat.CL, outputs_euler.CL, rtol=1e-3)
        np.testing.assert_allclose(
            outputs_quat.CD, outputs_euler.CD, rtol=1e-3)
        print('  ✓ Aerodynamic outputs match')

        print('\nForces and moments:')
        print(f'  Quaternion: F_b={outputs_quat.F_b}, M_b={outputs_quat.M_b}')
        print(f'  Euler:      F_b={
              outputs_euler.F_b}, M_b={outputs_euler.M_b}')
        np.testing.assert_allclose(
            outputs_quat.F_b, outputs_euler.F_b, rtol=1e-2, atol=1e-3)
        np.testing.assert_allclose(
            outputs_quat.M_b, outputs_euler.M_b, rtol=1e-2, atol=1e-3)
        ...
        A_quat, B_quat = linearize_dynamics(
            model_quat, x_trim_quat, u_trim_quat)
        A_euler, B_euler = linearize_dynamics(
            model_euler, x_trim_euler, u_trim_euler)

        print('\nState space dimensions:')
        print(
            f'  Quaternion: A is {A_quat.shape[0]}x{
                A_quat.shape[1]} (13 states: p[3], v[3], q[4], w[3])'
        )
        print(
            f'  Euler:      A is {A_euler.shape[0]}x{
                A_euler.shape[1]} (12 states: p[3], v[3], r[3], w[3])'
        )
        ...
        classified_euler = classify_aircraft_modes(
            modes_euler, state_names_euler)
        ...
        for key, name in mode_types:
            ...
            if mode_q is None and mode_e is None:
                ...
                print(
                    f"  Euler:      λ={
                        mode_e['eigenvalue_continuous']:.4f}, τ={
                        mode_e['time_constant']:.3f}s")
                continue
            elif mode_e is None:
                print(
                    f"  Quaternion: λ={
                        mode_q['eigenvalue_continuous']:.4f}, τ={
                        mode_q['time_constant']:.3f}s")
            ...
            if mode_q['is_oscillatory'] and mode_e['is_oscillatory']:
                ...
                # Check that frequencies match closely (should be identical for
                # physical modes)
                freq_diff = abs(mode_q['frequency_hz'] -
                                mode_e['frequency_hz'])
                freq_rel_diff = freq_diff / max(mode_q['frequency_hz'], 1e-6)
                if freq_rel_diff > 0.05:  # 5% tolerance
                    print(f'    ⚠ WARNING: Frequency mismatch {
                          freq_rel_diff * 100:.1f}%')
                else:
                    print(f'    ✓ Frequencies match (diff: {
                          freq_rel_diff * 100:.2f}%)')
            ...
            if tc_rel_diff > 0.05:  # 5% tolerance
                print(f'  ⚠ WARNING: Time constant mismatch {
                      tc_rel_diff * 100:.1f}%')
            else:
                print(f'  ✓ Time constants match (diff: {
                      tc_rel_diff * 100:.2f}%)')

            # Check stability agreement
            if mode_q['stable'] != mode_e['stable']:
                print('  ⚠ WARNING: Stability disagreement!')
            else:
