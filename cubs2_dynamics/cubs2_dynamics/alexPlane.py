...
class _Base:
    ...
    # Physical properties
    thr_max: float = param(15, desc="maximum thrust (N)") # What is this
    m: float = param(2, desc="mass (kg)")
    S: float = param(0.63, desc="wing area (m^2)")
    rho: float = param(1.225, desc="air density (kg/m^3)")
    g: float = param(9.81, desc="gravity (m/s^2)")

    # Inertias
    Jx: float = param(0.25303, desc="roll inertia (kg·m^2)")
    Jy: float = param(0.8166, desc="pitch inertia (kg·m^2)")
    Jz: float = param(1.03801, desc="yaw inertia (kg·m^2)")
    Jxz: float = param(0.09704, desc="product of inertia (kg·m^2)")

    # Geometry
    cbar: float = param(0.352, desc="mean chord (m)")
    span: float = param(1.8, desc="wingspan (m)")
    wing_incidence: float = param(
        np.deg2rad(3.0), desc="wing incidence angle (rad)"
    )  # Verify

    # Pitch coefficients
    Cm0: float = param(7.38e-2, desc="pitch moment coeff")
    Cma: float = param(-0.55758, desc="pitch moment slope (1/rad)")
    Cmq: float = param(-10.3742, desc="pitch damping (1/rad)")

    # Lift & drag
    CL0: float = param(0.3636, desc="lift coeff at zero AoA")
    CLa: float = param(4.524, desc="lift slope (1/rad)")
    ...
    # Stability & damping
    Cnb: float = param(0.13112, desc="yaw stiffness (1/rad)")
    CYb: float = param(-0.23, desc="sideslip sideforce (1/rad)")
    CYr: float = param(0.298, desc="yaw rate sideforce")
    CYp: float = param(0.065, desc="roll rate sideforce")
    Clb: float = param(-0.0871, desc="dihedral effect (1/rad)")
    Clp: float = param(-0.328, desc="roll damping")
    Clr: float = param(-0.0325, desc="yaw-roll coupling")
    Cnr: float = param(-0.149, desc="yaw damping")
    Cnp: float = param(0.00929, desc="roll-yaw coupling")
...
@beartype
def clamp(
    val: cy.SXType, low: cy.SXType | int | float, high: cy.SXType | int | float
) -> cy.SXType:
...
def casadi_min_withcargo(
    costs: list[cy.SXType], cargos: list[cy.SXType]
...
def _compute_ground_forces(
    p_sym,
    v_sym,
    w_sym,  # state: position, velocity, angular velocity
    ...
    ground_c_xy,
    ground_mu,
    ground_max_force_per_wheel,
    max_defl_rud,
    tailwheel_steer_gain,
) -> tuple[cy.SXType, cy.SXType]:
    ...
    for i, wheel_b in enumerate(wheel_b_list):
        ...
        lateral_damp = cy.vertcat(
            -vel_wheel_e[0] * ground_c_xy, -vel_wheel_e[1] * ground_c_xy
        )
        lateral_mag = cy.norm_2(lateral_damp) + 1e-9
        max_lateral = ground_mu * normal_force
        scale_lat = cy.if_else(
            lateral_mag > max_lateral, max_lateral / lateral_mag, 1.0
        )
        lateral_limited = scale_lat * lateral_damp

        force_contact_e = cy.vertcat(
...
def sportcub(attitude_rep: AttitudeRep = "quat") -> Model:
    ...
    FG_b, MG_b = _compute_ground_forces(
        ...
        p.ground_c_xy.sym,
        p.ground_mu.sym,
        p.ground_max_force_per_wheel.sym,
        p.max_defl_rud.sym,
        p.tailwheel_steer_gain.sym,
    )
    ...
    F_b = (
        (1.0 - p.disable_aero.sym) * FA_b
        + (1.0 - p.disable_gf.sym) * FG_b
        + FT_b
        + FW_b
    )
    M_b = (
        (1.0 - p.disable_aero.sym) * MA_b
        + (1.0 - p.disable_gf.sym) * MG_b
        + MT_b
        + MW_b
    )
