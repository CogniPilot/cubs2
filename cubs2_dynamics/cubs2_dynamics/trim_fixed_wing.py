...
def find_trim_fixed_wing(
    ...
    if hasattr(s_guess, "q"):
        s_guess.q = np.array([np.cos(pitch_guess / 2.0), 0.0, np.sin(pitch_guess / 2.0), 0.0])
    ...
    def aircraft_constraints_fn(
        ...
        if hasattr(x.r, "shape") and x.r.shape[0] == 4:
            ...
            constraints.append(x.r[0] >= 0.5)  # keep quaternion in positive hemisphere
...
@beartype
def classify_aircraft_modes(modes: list[dict], state_names: list[str]) -> dict[str, dict | list]:
    """Fixed-wing aircraft mode classification (short-period, phugoid, etc.)."""

    def contrib(mode: dict, pattern: str) -> float:
        return sum(mag for name, mag in mode.get("dominant_states", []) if pattern in name)
    ...
    def try_classify(mode: dict) -> str | None:
        ...
        if (contrib(mode, "v[0]") > 0.3 or contrib(mode, "v[2]") > 0.3) and v_c > 0.4:
        ...
        if osc and (contrib(mode, "v[1]") > 0.2 or contrib(mode, "w[2]") > 0.3):
    ...
    classified = {
        key: None for key in ["short_period", "phugoid", "dutch_roll", "roll_damping", "spiral"]
    }
    classified["other"] = []
